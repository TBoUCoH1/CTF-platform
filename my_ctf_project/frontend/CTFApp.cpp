...
void CTFApp::setupLoginPage() {
    ...
    // ОБНОВЛЕННЫЙ СТИЛЬ ДЛЯ ПОЛЕЙ ВВОДА С ПОДСВЕТКОЙ
    ...
    // Стиль для кнопки Sign In (primary)
    ...
    // Стиль для кнопки Create Account (secondary)
    ...
    // Enter для отправки формы логина
...
void CTFApp::setupSidebar() {
    ...
    // Стиль навигационных кнопок - dashboard style
    ...
    // Navigation buttons
    homeBtn = new QPushButton(QString::fromUtf8("🏠 Главная"));  // Home
    ...
    challengesBtn = new QPushButton(QString::fromUtf8("🎯 Задания"));  // Challenges
    ...
    leaderboardBtn = new QPushButton(QString::fromUtf8("🏆 Рейтинг"));  // Leaderboard
    ...
    profileBtn = new QPushButton(QString::fromUtf8("👤 Профиль"));  // Profile
...
void CTFApp::setupHomePage() {
    ...
    // ОБНОВЛЕННАЯ функция - теперь иконка и цифры используют accentColor
    ...
    // Создаем карточки с РАЗНЫМИ цветами
    ...
    // --- Left Column: Recommended Challenge ---
    ...
    // ОБНОВЛЕННАЯ секция с двумя badges
    QHBoxLayout *badgeRow = new QHBoxLayout();
    
    // Зелёный badge "Начальный этап"
    ...
    diffBadge->setMinimumWidth(120);  // Увеличили ширину
    diffBadge->setAlignment(Qt::AlignCenter);
    
    // Жёлтый badge "Старт" (как у Global Rank)
    ...
    // --- Right Column: Полезные инструменты ---
...
void CTFApp::setupLeaderboardPage() {
    ...
    // === Header таблицы УПРОЩЕННЫЙ (без "Обновлено только что") ===
...
void CTFApp::setupProfilePage() {
    ...
    int fixedWidth = 120;  // Одинаковая ширина для всех элементов
    ...
    *outValueLabel = valueLabel;
    ...
    // === Progress Section (ПРОЦЕНТ ВЫПОЛНЕНИЯ) ===
...
void CTFApp::showChallengesPage() {
    ...
    if (!tasksLoaded) {
        ...
        // Просто переустанавливаем фильтры на существующие виджеты
        challengeManager->installCardEventFilters(this);
    }
}
    ...
    connect(reply, &QNetworkReply::finished, this, [this, reply]() {
        if (reply->error() == QNetworkReply::NoError) {
            ...
            for (const auto &item : data) {
                ...
                // === РАНГ С ЦВЕТАМИ (QLabel по центру) ===
...
void CTFApp::onProfileResponse(QNetworkReply *reply) {
    if (reply->error() == QNetworkReply::NoError) {
        ...
        qDebug() << "Profile response:" << data;  // DEBUG - посмотрите что возвращает API
        
        QJsonObject profile = QJsonDocument::fromJson(data).object();
        
        currentScore = profile["score"].toInt();
        int rank = profile["rank"].toInt();      // Проверим что здесь
        int level = profile["level"].toInt();    // И здесь
        int solved = profile["solved_count"].toInt();
        int total = profile["total_tasks"].toInt();
        
        qDebug() << "Score:" << currentScore << "Rank:" << rank << "Level:" << level << "Solved:" << solved;  // DEBUG
        
        // Процент выполнения
        int progressPercent = total > 0 ? (solved * 100 / total) : 0;
        
        // Обновляем UI - используем level если rank = 0
...
void CTFApp::showError(const QString &title, const QString &message) {
    qDebug() << "Show error:" << title << message; // DEBUG
...
// Event filter для обработки кликов на карточках задач и Tab в flagInput
